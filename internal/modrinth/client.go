...
// Tags is the project's categories with the loader entries removed.
...
// SupportedLoaders is the other half of Categories: the loaders the project
// publishes for.
...
// Supports reports whether this build lists the given Minecraft version. It is
// what tells a "works here" release apart from one that merely exists, which
// matters as soon as the version filter is relaxed to see a project's history.
...
// GameVersionRange collapses the supported versions into "1.21" or "1.20–1.21.4".
...
// Channel is the release channel as a single character, for a list marker.
...
// SortIndexes are the orderings Modrinth's search accepts, in the order the UI
// cycles them. "relevance" with an empty query is what the site's front page
// shows, which is how browsing starts before anything has been typed.
var SortIndexes = []string{"relevance", "downloads", "follows", "newest", "updated"}

// SortLabel names an index the way the website does.
...
// Versions lists a project's versions, newest first, optionally narrowed to the
// loaders and Minecraft versions the server actually runs.
...
// LatestVersion picks the newest release for the given loaders and game
// version, falling back to betas only when there is no release at all.
