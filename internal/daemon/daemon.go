...
// request runs a control command and answers the caller with the outcome, so
// one-shot CLI invocations get a real result instead of fire-and-forget.
...
// Spawn launches a detached daemon for dir and waits until its socket answers.
