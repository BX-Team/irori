...
type ServerSection struct {
	...
	// BuildID is the mcjars uuid of the build. It is what identifies the
	// configuration files this core ships with, so irori can restore them.
...
// Find walks up from start looking for .irori.json and returns the directory
// holding it.
...
// LoadFrom finds the nearest config walking up from start and loads it.
...
// SocketPath keeps the socket next to the server, falling back to the runtime
// dir when the path would exceed the sun_path limit of a unix socket address.
...
// SetOverride records a declared key for a config file so `irori apply` keeps
// enforcing it.
