...
// sync pads short output with leading blank lines so the console reads bottom
// up like a real terminal instead of hanging from the top of the panel.
func (c *Console) sync() {
	content := c.rendered
...
// render styles one console line and wraps it to the viewport width, dimming
// the timestamp/thread prefix so the message itself stands out.
