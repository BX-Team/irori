...
// List is a filterable single-selection list used by the wizard, the plugin
// browser and the settings screens.
type List struct {
	s        *Styles
	items    []ListItem
	visible  []int
...
func (l *List) clampOffset() {
	...
	// Also pull back when the window grew or the list shrank, so the last screen
	// is never padded with blank rows.
...
// ScrollBadge is a compact "12/340" indicator for a panel title bar.
