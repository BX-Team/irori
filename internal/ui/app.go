...
// forward broadcasts state — link events, logs, config changes — to every
// screen, so a background tab is up to date the moment it is opened.
...
// power routes a power action: when no daemon is listening a start must first
// spawn one, since the daemon is what owns the server process.
