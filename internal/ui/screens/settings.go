...
// rebuild refreshes the form from the config while keeping values the user has
// edited but not saved yet; reset throws those edits away.
...
// requiredNote spells out where the floor came from, since a zero in the field
// is not the same as no requirement at all.
...
// preview renders the command line the current field values would produce, so a
// flag change is visible before it is saved. It grows to fit the flags, capped
// at half the screen.
