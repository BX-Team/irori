...
// sealedNote explains why the network side of this screen is inert; on NixOS
// plugins come from the store and are declared in configuration.nix.
...
// Init opens on Modrinth's front page rather than an empty pane: a plain
// search with no query is exactly what the website shows, so browsing what is
// popular for this server needs no typing at all.
...
// newSearch starts over from the first page; paging keeps the offset.
...
// gameVersion is the Minecraft version the query is narrowed to, or "" when the
// user has relaxed that filter to see what else a project offers.
...
// install downloads the jar, then records the plugin in both .irori.json and the
// lock file so `irori apply` reproduces exactly this state elsewhere.
...
// declare adopts a jar that was dropped into the directory by hand, so apply
// stops calling it untracked and keeps it around.
...
func (p *Plugins) setVersions(versions []modrinth.Version) {
	...
	for _, v := range versions {
		// The marker answers the only question that matters before installing:
		// will this build run on the version this server is on. It is not
		// always yes even when the query asked for it — a relaxed filter shows
...
// cycleSort walks the orderings Modrinth offers, re-running the current query.
...
// toggleVersionFilter relaxes or restores the Minecraft version facet. A search
// that comes back empty is almost always this filter's doing — a plugin that
// has not been rebuilt for the newest release yet still exists — and being able
// to see that is more useful than an empty pane.
...
// page steps through the result set. Modrinth reports the full hit count, so
// walking off the end can be refused rather than shown as an empty list.
...
func (p *Plugins) browsePanel(width int) string {
	...
	// The project detail earns its space only once the pane is tall enough that
	// the list above it still shows a useful number of rows.
	detailHeight := 0
	if p.Height >= 22 {
...
// browseTitle says what the pane is showing, since an untyped search is the
// front page rather than a search for nothing.
...
// facetLine spells out the filters applied automatically, so an empty result
...
// projectDetail is the row of facts a list line has no room for: who wrote it,
// what it is tagged as, and whether it is a server-side thing at all.
func (p *Plugins) projectDetail(width, height int) string {
	...
	// A client-only mod installed on a server does nothing, so say so plainly.
...
// versionsView stacks the build list over a detail pane. Picking a version is
// the one place where the numbers alone are not enough: what decides it is
// which loaders and Minecraft versions the build declares, and what changed.
...
func (p *Plugins) versionDetail(width, height int) string {
	...
	// Whatever rows are left go to the changelog, which is the reason to look
	// at an older or newer build in the first place.
...
// shortChangelog flattens the markdown Modrinth stores into something readable
// in a few terminal rows: headings and list bullets become plain text, links
// keep their label, and the rest collapses onto one paragraph.
