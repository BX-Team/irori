...
// Configs is the structured editor for every configuration file the server
// core ships: server.properties, the Bukkit and Paper YAML family, whatever a
// fork adds under config/, and a proxy's velocity.toml. Keys carry the comment
// their own file gives them, so nothing here needs a hand-written catalog
// beyond the one server.properties already has.
type Configs struct {
	Base
	cfg *config.Config
	h   host.Backend
	...
	// current is the path of the file on screen, "" when none could be loaded.
	current string
...
// discover rebuilds the file picker and opens the first file, preferring
// server.properties since that is what people come to this tab for.
...
// select_ moves to a file, parking the current file's unsaved edits first.
...
// stash copies the live widget state back into the open document, so it is the
// widget and not the map that is treated as authoritative while a file is on
// screen.
...
// refreshBadges keeps the unsaved count next to each file in the picker, which
// is the only signal that a file left behind still has pending edits.
...
// toggleDeclared moves a key in or out of .irori.json so `irori apply` either
// enforces it or leaves it alone.
...
// typedValue keeps .irori.json readable: booleans and numbers are stored as JSON
// scalars rather than quoted strings.
...
func (c *Configs) View() string {
	...
	// Below three columns the panes take turns, so the one with focus gets the
	// whole width rather than both being cut in half.
...
// summary says what the file is when there is nothing more urgent to report,
// since a bare key list gives no clue which of a dozen YAML files is on screen.
...
func (c *Configs) detail() string {
	...
	// Which block the key sits in, so the reader is not lost once the section
	// header has scrolled off the top of the list.
	if f.Section != "" {
...
// tailPath keeps the end of a dotted path, which is the part that identifies
// the block. A Paper world config nests six levels deep, and wrapping the whole
// thing costs five lines of a narrow column to say almost nothing.
