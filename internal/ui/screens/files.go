...
// filesDeleteMsg travels through the shell's confirmation modal and comes back
...
// restoreDefaults puts a config file back to what the core ships, which is the
// way out of an edit the server refuses to start from. The list of shipped
// files is fetched once and then reused.
