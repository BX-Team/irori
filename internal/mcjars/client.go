...
// CoreJar reports the jar the recipe finally leaves behind, so callers point at
// the right file for server types whose installer renames or unpacks things.
...
// Types returns every server type grouped by the category mcjars files it
// under, in a stable presentation order.
...
// Match is a jar mcjars recognised: the build it is, plus the version record
// that build belongs to. The version is what carries the required JDK, which
// the build alone does not know.
type Match struct {
	Build   Build
	Version Version
}
...
// Config is a configuration file the core ships with, as mcjars records it.
type Config struct {
	Location string `json:"location"`
...
// Configs returns the pristine configuration files for a build, keyed by the
// path they belong at. It is what lets irori put back a config someone edited
// into a state the server will not start from.
