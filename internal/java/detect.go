...
// Resolve picks a JDK for the server: an explicit path always wins, otherwise
// the cheapest sources are probed first and the nix store is scanned only as a
// last resort.
...
// nixCandidates reads the major version out of the store path so only the most
// promising jdk is actually executed; probing every entry would be slow on a
// machine with a large store.
...
// MajorOf turns both legacy ("1.8.0_392") and modern ("21.0.5") version
// strings into a major number.
...
// InstallHint suggests how to obtain a missing JDK, preferring nix when the
// machine looks like a nix host.
