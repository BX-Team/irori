...
// Build produces the right-hand preview for one entry. It reads at most a
// window of the file, so opening a directory of large logs stays cheap.
func Build(h host.Backend, p string, e host.Entry, maxLines int) Preview {
	...
	// A cut-off read usually leaves a partial last line.
...
// looksTextual rejects NUL bytes and long stretches of invalid UTF-8, which is
// enough to keep jars, images and world data out of the text preview.
