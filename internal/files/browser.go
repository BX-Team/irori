...
// Load reads a directory. Paths outside the root are clamped back to it.
...
// Sort puts directories first and orders each group case-insensitively, the
// arrangement file managers converge on.
...
// Clean normalises a browser path: slash separated, no leading slash, never
// above the root.
...
// Paste applies the clipboard into the current directory, renaming collisions
// to "name (copy)" rather than overwriting anything.
